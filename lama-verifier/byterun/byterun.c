...
static inline void vstack_check_size(size_t count) {
...
stack_frame *init_call_stack() {
  stack_frame *frame = (stack_frame *)calloc(1, sizeof(stack_frame));
  frame->base_ptr = (int *)vstack_after_top();
  return frame;
}
...
void interpret(const bytefile *bf, char *end_file, const char *fname) {
  ...
  do {
    ...
    case CBINOP: {
      ...
      vstack_push(BOX(binop((BinopCode)(l - 1), left, right)));
      break;
    }

    case CLD: {
      int ind = INT;
      int *addr = (int *)op_stack_load_addr(frame, (location)l, ind);
      vstack_push(*addr);
      #ifdef DEBUG
      debug_ld(f, h, l, ind);
      #endif
    ...
    default:
      switch (x) {
        ...
        case  CSEXP: {
          ...
          int count     = INT;
          size_t value = (size_t)Bsexp(count, UNBOX(LtagHash((char *)name)));
          vstack_push(value);
        ...
        case  CBEGIN:
        case  CCBEGIN: {
          int stack_growth = INT >> 16, 
              local_count = INT;
          vstack_check_size(stack_growth);
        ...
        case  CCLOSURE: {
          ...
          for (int i = 0; i < n; i++) {
            int typ = BYTE,
                ind = INT;
            int *addr = (int *)op_stack_load_addr(frame, (location)typ, ind);
            vstack_push(*addr);
          };
        ...
        case  CCALLC: {
          ...
          frame = stack_frame_call(frame, (int *)vstack_after_top(), ip, true, arg_count);
        ...
        case  CCALL: {
          ...
          int arg_count = INT;
          vstack_reverse_count(arg_count);
          int *base_ptr = (int *)vstack_after_top();
        ...
        case  CTAG: {
          const char *name    = STRING;
          int arg_count = INT;
          size_t tag = Btag((void *)vstack_pop(), LtagHash((char *)name), BOX(arg_count));
          vstack_push(tag);
...
int main (int argc, char* argv[]) {
  ...
  if (!(argc == 3 && strcmp(argv[2], "--noverify") == 0) && !verify(f)) {
    printf(stderr, "Verify error\n");
    return 1;
  }
  interpret(f, f->end, argv[1]);
  close_file(f);
  return 0;
}
